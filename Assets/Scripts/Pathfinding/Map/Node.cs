...
public class Node {
	...
	public void Initialize(float x, float y, Vector2 position, Grid grid)
	{
		...
		OtherDebug = GameObject.Instantiate (Resources.Load ("Node")) as GameObject; 
		OtherDebug.transform.position = Position;
		OtherDebug.GetComponent<OtherDebug> ().X = X;
		OtherDebug.GetComponent<OtherDebug> ().Y = Y;
	}

	public void SetColor(Color color)
	{
		OtherDebug.transform.GetComponent<SpriteRenderer> ().color = color;
	}
